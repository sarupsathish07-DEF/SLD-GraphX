...
Local-first electrical SLD intelligence for RMKECIHS93. Milestone 2 adds real, local OCR evidence: PNG/JPEG/PDF pages can be recognized through an isolated PaddleOCR worker, normalized into conservative electrical text types, persisted with confidence/provenance, and reviewed in the engineering workspace. It does **not** claim symbol detection, conductor reconstruction, or graph extraction from uploaded drawings.
...
See [Milestone 2 receipt](docs/milestone-2-receipt.md) and [limitations](docs/limitations.md) for evidence and boundaries.
